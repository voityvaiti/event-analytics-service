...
# Read cell: GET /api/v1/stats/event-counts, one measured run per grouping.
# Defines perf_read_event_counts; the harness and perf/read/measure-read.sh must
# already be sourced. Appends one row per grouping to
# perf/read/event-counts/journal.jsonl.
#
...
perf_read_event_counts() {
  local journal=perf/read/event-counts/journal.jsonl
  local grouping status=0

  for grouping in type hour day; do
    perf_read_cell "$journal" event-counts "$grouping" || status=$?
  done
