...
- **Hikari connection pool size.** Blocking JDBC on virtual threads means each
  ...
  (`SPRING_DATASOURCE_HIKARI_MAXIMUM_POOL_SIZE=20 ./gradlew bootRun`); the
  harness reads the running app's actual pool from its actuator metrics and
...
itself is **not** installed on the host — the harness runs it from a pinned
